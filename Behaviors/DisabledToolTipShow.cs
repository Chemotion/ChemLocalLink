...
public static class ShowDisabledTooltipExtension
{
  ...
  private static void TopLevel_PointerMoved(object sender, global::Avalonia.Input.PointerEventArgs e)
  {
    ...
    var attached_controls = tl.GetVisualDescendants().Where(x => GetShowOnDisabled(x)).Cast<Control>();

    // find disabled children under pointer w/ this extension enabled
    var disabled_child_under_pointer = attached_controls.FirstOrDefault(x =>
      x.Bounds.Contains(e.GetPosition(x.Parent as Visual)) && x.IsEffectivelyVisible && !x.IsEnabled
    );
    ...
    var disabled_tooltips_to_hide = attached_controls.Where(x =>
      ToolTip.GetIsOpen(x) && x != disabled_child_under_pointer && !x.IsEnabled
    );
